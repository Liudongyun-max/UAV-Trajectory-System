...
* 📈 **多特征轨迹跟踪 (Feature Tracker)**：
  ...
  * **一阶指数平滑滤波**：引入一阶低通滤波 ($\alpha = 0.65$) 对 BBox 的中心点及宽高进行防抖修正，画面定位框锁定极其平稳。
* 🧠 **GRU 时序预测网络**：
...
├── PROJECT_DOCUMENTATION.md   # 项目系统架构全景审计说明书 (3.0版)
...
# 运行训练脚本
python scripts/train.py --config configs/gru_enhanced.yaml
```
